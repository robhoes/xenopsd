...
module Storage = struct
	...
	let id_of vm vbd = Printf.sprintf "vbd/%s/%s" vm (snd vbd)
...
(*						(fun () -> Device.Vif.release task ~xs device) *)
...
(*
	let create_device_model_exn vbds vifs saved_state xc xs task vm di =
		...
		let qemu_dm = !Path.qemu_dm_wrapper (* choose_qemu_dm vm.Vm.platformdata *) in
		let xenguest = !Path.xenguest (* choose_xenguest vm.Vm.platformdata *) in
...
*)
	let create_device_model task vm vbds vifs saved_state = ()
	...
	let build ?restore_fd task vm vbds vifs =
		...
		with_xc_and_xs (fun xc xs ->
			...
			Mem.with_reservation task.Xenops_task.dbg min_kib max_kib (fun target_plus_overhead_kib reservation_id ->
				...
				in
				let vkbs = [||]
				(*	let open Xenlight.Device_vkb in
					match vm.Vm.ty with
					| Vm.PV _ -> [| default |]
					| Vm.HVM _ -> [||]*)
				in
...
*)
				with_data ~xc ~xs task data true
					...
					(*	let di = Xenctrl.domain_getinfo xc domid in
						let pages = Int64.of_nativeint di.Xenctrl.Domain_info.total_memory_pages in *)
